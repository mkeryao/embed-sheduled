...
/**
 * Entity representing the configuration for a schedulable task.
 * This includes cron expression, task type, execution parameters,
 * advanced scheduling rules (like exclusions, timeouts), notification settings,
 * and workflow definitions if the task is a workflow.
 */
@Data
@NoArgsConstructor
@AllArgsConstructor
public class TaskConfig {
     ...
     * 1: HTTP task (makes an HTTP request - Not Implemented).
     * 2: Shell script (executes a shell script - Not Implemented).
    ...
    /** JSON string representing parameters for the bean method (for Bean tasks). */
    private String beanParameters;

    // (HTTP and Shell parameters are placeholders as these types are not fully implemented)
    ...
    /** Optional name of a distributed lock to acquire before executing the task. */
    private String taskLockName;
    /** Duration in seconds for which the distributed lock should be held. */
    private Integer taskLockMostSeconds;

    // --- Advanced Scheduling Features ---
    /** 
     * Name of a {@link TaskCalendar#calendarName} to check for non-working days. 
     * If the current day is a non-working day in this calendar, the task will be skipped.
     */
    ...
    /** Comma-separated user IDs (from {@link TaskUser#userId}) to notify upon successful task completion. */
    private String notifySuccessUserIds;
    /** Comma-separated user IDs to notify upon failed or timed-out task completion. */
    private String notifyFailedUserIds;

    // --- Workflow Definition (if taskType is WORKFLOW) ---
    /** JSON string representing an array of {@link com.example.taskscheduler.dto.workflow.WorkflowNode} objects. */
    private String workflowNodesJson;
    /** JSON string representing an array of {@link com.example.taskscheduler.dto.workflow.WorkflowEdge} objects. */
